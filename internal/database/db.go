...
type DB interface {
	...
	GetEntity(tableName string) ([]StructWithTableName, error)
	GetEntityWithoutTableName() ([]StructWithTableName, error)
}

// StructWithTableName is structure of creating models base on table in database
type StructWithTableName struct {
	Models     string
	TableName  string
	StructName string
}
...
// GetEntity ...
func (s *store) GetEntityWithoutTableName() ([]StructWithTableName, error) {
	query := getReverseWithoutTbale()
	log.Info(query)
	rows, err := s.db.Query(query)
	if err != nil {
		return []StructWithTableName{}, err
	}
	structs := make([]StructWithTableName, 0)
	for rows.Next() {
		var tmpl string
...
// GetEntity ...
func (s *store) GetEntity(tableName string) ([]StructWithTableName, error) {
